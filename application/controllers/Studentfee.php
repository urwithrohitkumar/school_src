...
class Studentfee extends Admin_Controller
{
    ...
    public function getcollectfee()
    {
        ...
        foreach ($record_array as $key => $value) {
            ...
            $feeList               = $this->calculateFine($feeList);
    ...
    public function getStuFeetypeBalance($fee_groups_feetype_id, $student_fees_master_id)
    {
        ...
        if (strtotime($result->due_date) < strtotime(date('Y-m-d'))) {
            $result = $this->calculateFine($result);
    ...
    public function reportduefees()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            $data['class_id']   = $class_id;
            $data['section_id'] = $section_id;
            $fees_dues          = $this->studentfeemaster_model->getStudentDueFeeTypesByDate($date, $class_id, $section_id, $branch_id);
            $students_list      = array();
     ...
     * @return stdClass
     */
    private function calculateFine(stdClass $fine): stdClass
    {


        if (!empty($fine)) {
            switch ($fine->fine_term_id) {
                case '1':
                    $days =  app_date_diff($fine->due_date, date('Y-m-d'));
                    $fine->fine_amount = $fine->fine_amount * $days;
                    break;
                default:
                    # code...
                    break;
