...
//==========================
            ...
            if ($insert) {
                ...
                if ($staff_id) {
                    ...
                    $teacher_login_detail = array('id' => $staff_id, 'credential_for' => 'staff', 'username' => $email, 'password' => $password, 'mobileno' => $contact_no, 'email' => $email,'contact_no'=>'');

                    $this->sms_send->mailsms('login_credential', $teacher_login_detail);
                }
                            ...
                            if ($check_exists == 0 && $check_emailexists == 0) {
                                ...
                                if ($staff_id) {
                                    ...
                                    $teacher_login_detail = array('id' => $staff_id, 'credential_for' => 'staff', 'username' => $result[$r_key]['email'], 'password' => $password, 'contact_no' => $result[$r_key]['contact_no'], 'email' => $result[$r_key]['email']);
                                                        ...
                                                        sendSatfflogin($teacher_login_detail);
