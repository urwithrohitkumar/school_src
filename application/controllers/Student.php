...
class Student extends Admin_Controller
{
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($insert) {
                ...
                $data_student_login = array(
                    'username' =>  strtolower($userName) . $insert_id,
                ...
                if ($sibling_id > 0) {
                    ...
                    $temp              = $insert_id;
                    // $this->parent_login_prefix 
                    $parentName = $this->input->post('guardian_name');
                    $data_parent_login = array(

                        'username' => strtolower($parentName) . $insert_id,
                ...
                $this->sms_send->mailsms('student_admission', $sender_details);
                ...
                $this->sms_send->mailsms('login_credential', $student_login_detail);
                ...
                if ($sibling_id > 0) {
                    ...
                    $this->sms_send->mailsms('login_credential', $parent_login_detail);
                }
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            if (!empty($insert_id)) {
                                ...
                                $user_password = '123456';
                                $sibling_id    = $this->input->post('sibling_id');
                                //$this->student_login_prefix
                                $data_student_login = array(
                                    'username' => strtolower($student_data[$i]["firstname"]) . $insert_id,
                                    'password' => $user_password,
                                    'user_id'  => $insert_id,
                                    'role'     => 'student',
                                );
                                ...
                                $data_parent_login = array(
                                    'username' => strtolower($student_data[$i]["guardian_name"]) . $insert_id,
                                    'password' => $parent_password,
                                    'user_id'  => $insert_id,
                                    'role'     => 'parent',
                                    'childs'   => $temp,
                                ...
                                $sender_details = array(
                                    'student_id' => $insert_id,
                                    'contact_no' => $student_data[$i]["guardian_phone"],
                                    'email' => $this->input->post('guardian_email'),
                                    'firstname' => $this->input->post('firstname'),
                                    'mobileno'   => $student_data[$i]["mobileno"],
                                    'class_id'      => $class_id,
                                    'section_id'    => $section_id,
                                    'session_id'    => $session
                                );
    ...
    public function sendbulkmail()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = array('status' => 1, 'error' => '', 'message' => $this->lang->line('message_sent'));
        }
        echo json_encode($array);
    }
