            ...
            success: function (res)
            {
                ...
                if (res.status == 0) {
                    ...
                    if (res.result.file_name != null) {
                        var string = res.result.file_name;
                        var response = string.split(',');
                        for (var i=0; i<response.length; i++) {

                               var newListItem =  '<div class=""><div class="col-sm-12"><div class="form-group"><label for="pwd"><?php echo $this->lang->line('uploaded') . " " . $this->lang->line('documents'); ?></label><p>' +response[i]+ ' <a href="<?php echo base_url(); ?>/user/homework/assigmnetDownload/' + id + '/' + response[i] + '" class="btn btn-default btn-xs" data-toggle="tooltip" data-original-title="<?php echo $this->lang->line('assignments') . " " . $this->lang->line('download'); ?>"><i class="fa fa-download"></i></a></p></div></div></div>';
                                $( "#uploaded_docs" ).append( newListItem );
                        }
                    }
