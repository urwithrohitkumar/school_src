...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function add($data)
    {
        ...
        $this->db->where('session_month', $data['session_month']);
    ...
    public function getCurrentSessionStudentFees($branch_id)
    {
    ...
    public function getStudentDueFeeTypesByDate($date, $class_id = null, $section_id = null, $branch_id = null)
    {
        $where_condition = array();
        if ($class_id != null) {
            $where_condition[] = " AND student_session.class_id=" . $class_id;
     ...
     * @return array
     */
    private function calculateFine($fine = []): array
    {

        if (!empty($fine)) {
            switch ($fine[0]->fine_term_id) {
                case '1':
                    $days =  app_date_diff($fine[0]->due_date, date('Y-m-d'));
                    $fine[0]->fine_amount = $fine[0]->fine_amount * $days;
                    break;
