                    ...
                    <div class="test">
                        ...
                        <div class="dualbg">
                            ...
                            <ul multiple=""  class="list-group" id="slist">
                                                            <lablel><input type="checkbox" name="chkSelectAll" class="checkAll" data-checkwhat="student_list"> Select All</lablel>
                                ...
                                foreach ($studentlist as $key => $value) {
                                    ...
                                    <li class="list-group-item <?php echo ($active_status) ? "active" : "" ?>">
                                        <div class="checkbox">
                                            <label><input class="student_list"  type="checkBox" <?php echo ($active_status) ? "checked='checked'" : "" ?>name="student_list[<?php echo $value["id"] ?>]" value="<?php echo $value["homework_evaluation_id"] ?>">
                                                <?php 
    ...
    <div class="col-lg-4 col-md-4 col-sm-4 col-eq">
        <div class="taskside">
            ...
            <div class="task-info task-single-inline-wrap ptt10">
                ...
                $document = '';
                $documentGet = explode(",",$result["document"]);
                foreach($documentGet as $documentRow) {
                $documentRow = trim($documentRow);
                ?>
                
                    <label>
                        <a href="<?php echo base_url() . "homework/download/" . $result["id"] . "/" . $documentRow ?>"><?php echo $this->lang->line('download') ?> <i class="fa fa-download"></i></a> &nbsp;&nbsp;&nbsp; <a  onclick="preview('<?php echo base_url() . "homework/download/" . $result["id"] . "/"  ?>','<?= $documentRow ?>');" >Preview <i class="fa fa-eye" aria-hidden="true"></i></a>  <?= $documentRow ?>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div style="overflow: hidden;" id="imgfile">
        
        <br><br>
        <button class="btn btn-primary" id="in">+</button>&nbsp;&nbsp;&nbsp;
        <button  class="btn btn-danger" id="out">-</button>
        </div>
        <div id="videofile">
        </div>
        <div id="pdffile">
...
<script>
    ...
    function listbox_moveacross(sourceID, destID) {
        ...
        function preview(url,file){
            ...
            $('#previewNot').empty();
            var url = url;
            var filePreview = file
                var extension = filePreview.substr( (filePreview.lastIndexOf('.') +1) );
                
                if(extension=='png' || extension=='jpg' || extension=='jpeg' || extension=='webp' || extension=='svg'|| extension=='gif' || extension=='bmp'|| extension=='jpe' || extension=='jfif' ){
                    $('#imgfile').prepend('<img id="previewImg" style="max-height: 300px;min-height: 300px"  width="320px" height="240px"    src="'+url+''+filePreview+'" />');

                    
                }else if(extension=='mp4'){
                                $('#imgfile').empty();

                    var video = `<video width="100%" height="auto" autoplay="true" controls><source src=${url}${filePreview} type="video/mp4"> <source src=${url}${filePreview} type="video/ogg">Your browser does not support the video tag.</video>`;
                $('#videofile').append(video);
                }
                else{
                     $('#imgfile').empty();

                   $("#previewNot").append('<h1>Preview not available  please download the file.</h1>');
                }
         ...
         $('#myModal123').on("hide.bs.modal", function(){
                $('#videofile').empty();
    ...
    $(document).ready(function(){
           $("#in").click(function(){
                $("#previewImg").width($("#previewImg").width()+100);
                $("#previewImg").height($("#previewImg").height()+100);
           });
           $("#out").click(function(){
                $("#previewImg").width($("#previewImg").width()-100);
                $("#previewImg").height($("#previewImg").height()-100);
