...
class Studentfee_model extends MY_Model
{
    ...
    public function __construct()
    {
        ...
        $this->current_user_id = ($this->session->has_userdata('admin')) ? $this->session->userdata('admin')['id'] : '0';
    }
    ...
    public function remove($id, $sub_invoice, $comments = '', $student_id)
    {
        ...
        if ($q->num_rows() > 0) {
            $result = $q->row();
            // r_data for refund record
            $r_data['receipt_number'] = $this->getRefundReceiptNumber();
            $r_data['student_fees_master_id'] = $result->student_fees_master_id;
            $r_data['fee_groups_feetype_id'] = $result->fee_groups_feetype_id;
            $r_data['amount_detail'] = $result->amount_detail;
            $r_data['is_active'] = 'yes';
            $r_data['remark'] = $comments;
            $r_data['student_id'] = $student_id;
            $r_data['refund_by'] = $this->current_user_id;
            ...
            if (!empty($a)) {
                ...
                $action = "Delete";
                $record_id = $id;
                $this->db->insert('student_fees_refund', $r_data);
                $this->log($message, $record_id, $action);
            }
        }
        //======================Code End==============================
    ...
    public function getDueStudentFees($branch_id = null, $feegroup_id = null, $fee_groups_feetype_id = null, $class_id = NULL, $section_id = NULL) {
    ...
    /**
     * This function is used to get refund receipt number
     *
     * @return string
     */
    private function getRefundReceiptNumber(): string
    {

        $receiptNumber = 'R' . date('ymd') . time();

        $this->db->select_max('id');
        $res1 = $this->db->get('student_fees_refund');

        if ($res1->num_rows() > 0) {
            $rId = $res1->row();
            if ($rId->id) {
                $receiptNumber .= $rId->id;
            }
        }
    ...
    /**
     * This function is used to get list of refund.
     *
